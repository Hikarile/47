    ...
    <script>
		...
		$(function(){
			...
			$('.col').each(function(){
				...
				$(this).find('div:first').draggable({revert:'invalid'});
            });
			$('.col').droppable({
				drop:function(e,u){
					var t=$(this);
			...
			$("#again,#auto,#back").submit(function(){
				$.ajax({
					url:"session.php"
				})
			})
		...
		function btn(from,to){
			var brick=$("#col"+from+" div:first").attr('data-id');
			if(brick==undefined){
				return false;
			}
			$.ajax({
		...
		function time(){
			if(auto){
				from='<?=$_SESSION['autofrom'][$_SESSION['count']]?>';
				to='<?=$_SESSION['autoto'][$_SESSION['count']]?>';
				brick='<?=$_SESSION['autobrick'][$_SESSION['count']]?>';
			}else if(again){
				from='<?=$_SESSION['from'][$_SESSION['again']]?>';
				to='<?=$_SESSION['ato'][$_SESSION['again']]?>';
