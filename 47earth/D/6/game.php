		...
		if($_SESSION['again']!=NULL || $_SESSION['auto']!=NULL){
			if($_SESSION['again']!=NULL){
				...
				if($_SESSION['auto']==$_SESSION['allcount']){
					$_SESSION['stime']=date("H:i:s");
					$_SESSION['auto']=NULL;
					header('lcoation:game.php?nickname='.$_SESSION['name'].'&difficulty='.$_SESSION['level'].'&fromStackId='.$_SESSION['from'][$_SESSION['count']].'&toStackId='.$_SESSION['to'][$_SESSION['count']].'&brickId='.$_SESSION['brick'][$_SESSION['count']]);
				}else{
					$_SESSION['auto']++;
				}
			}
