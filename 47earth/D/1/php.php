<?php
	class game{
		...
		public function reset($mlevel,$mfrom,$mto,$mbrick){//一開始
		...
		public function error($mname,$mlevel, $mfrom, $mto, $mbrick){//錯誤判斷
			if($mname<>$_SESSION['mname']){
				return"不可擅自更改姓名";
			}
			if($mlevel<>$_SESSION['mlevel']){
				return"不可擅自更改困難度";
		...
		public function go($mlevel,$mfrom,$mto,$mbrick){//移動
			if($_SESSION['back'] == 'true'){
				unset($_SESSION['back']);
				return false;
			}
