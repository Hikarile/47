	...
	class game{
		...
		public function star(){
			if($_SESSION['name']==''){
				...
				$_SESSION['bar'][0]=array(1=>$this->level,0,0);
				for($i=1;$i<=$this->level;$i++){
					$a[$i]=$i;
					$b[$i]=0;
				}$_SESSION['box'][0]=array(1=>$a,$b,$b);
		...
		public function go(){
			if(!($_SESSION['name']==$this->name&&$_SESSION['level']==$this->level&&$_SESSION['from'][$_SESSION['count']]==$this->from&&$_SESSION['to'][$_SESSION['count']]==$this->to&&$_SESSION['brick'][$_SESSION['count']]==$this->brick)){
				...
				$fff=$this->brick;
				foreach($box[$this->to] as $key=>$val){
					if($val!=0){
						if($ttt==''){
							$ttt=$val;
						}
		...
		public function back(){
			$_SESSION['count']--;
			header("location:game.php?nickname=".$_SESSION['name']."&difficulty=".$_SESSION['level']."&fromStackId=".$_SESSION['from'][$_SESSION['count']]."&toStackId=".$_SESSION['to'][$_SESSION['count']]."&brickId=".$_SESSION['rick'][$_SESSION['count']]);
			exit();
		}
