...
<head>
    ...
    <?php
		...
		if($_SESSION['out']!=''){
			if($_SESSION['out']=='out'){
				...
				$_SESSION['out']='out';
				header("location:game.php?nickname=".$_SESSION['name']."&difficulty=".$_SESSION['level']."&fromStavkId=".$_SESSION['from'][$_SESSION['count']]."&toStavkId=".$_SESSION['to'][$_SESSION['count']]."&brickId=".$_SESSION['brick'][$_SESSION['count']]);
			}
		}
		
		$name=$_GET['nickname'];
		$level=$_GET['difficulty'];
		$from=$_GET['fromStavkId'];
		$to=$_GET['toStavkId'];
		$brick=$_GET['brickId'];
	...
	<script>
		...
		$(function(){
			...
			$('.col').droppable({
				drop:	function(e,u){
					...
					$.ajax({
						...
						success: function(){
							location.change({
								fromStavkId:drag.parent().data('id'),
								toStavkId:	t.data('id'),
			...
			$("#auto,#again,#back").click(function(){
				$.ajax({
					url:"session.php",
				})
			})
		...
		function btn(from,to){
			var birck=$("#col"+from+" div:first").data('id');
			if(birck==undefined){
				return false;
			}
			$.ajax({
		...
		function time(){
			if(again!=''){
				...
				birck='<?=$_SESSION['birck'][$_SESSION['again']]?>';
			}else if(auto!=''){
				from='<?=$_SESSION['autofrom'][$_SESSION['auto']]?>';
				to='<?=$_SESSION['autoto'][$_SESSION['auto']]?>';
				birck='<?=$_SESSION['autoirck'][$_SESSION['auto']]?>';
			}
			$.ajax({
				url:"session.php",
				success: function(){
