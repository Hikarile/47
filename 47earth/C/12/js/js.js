$(function(){
	...
	var startcanvas=new Canvas(pen,canvas);
	...
	$("#save2").click(function(){
		var data={shapes:startcanvas.shapes};
	...
	$("#open2").change(function(){
		...
		reader.onload=function(event){
			...
			for(i=0;i<text.shapes.length;i++){
				...
				startcanvas.shapes.push(shape);
				if(i==text.shapes.length-1){
					startcanvas.valid=true;
					startcanvas.draw();
				}
			}
		}
...
function Canvas(pen,canvas){
	...
	$(canvas).mousedown(function(e){
		...
		if(tcanvas.mousedraw){
			...
			if(tcanvas.pen.type=='shape' && tcanvas.pen.shape=='no'){
				...
				if(tcanvas.mousezoom){
					shape.r=Math.abs(mouse.x-shape.x);
		...
		tcanvas.select='';
		tcanvas.mousezoom=tcanvas.mousemove=tcanvas.mousedraw=false;
	})
...
Canvas.prototype.draw=function(){
	if(this.valid){
		...
		image=ctx.getImageData(0,0,this.width,this.heigth);
