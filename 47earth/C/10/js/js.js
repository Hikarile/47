...
function Canvas(pen,canvas){
	...
	$(canvas).mousedown(function(e){
			...
			for(i=tcanvas.shapes.length-1;i>=0;i--){
				...
				if(mouse.x>shape.sx && mouse.y>shape.sy && mouse.x<shape.ex && mouse.y<shape.ey){
					...
					if(shape.shape!='g' && shape.shape!='line'){
						if( mouse.x<shape.ex && mouse.y<shape.ey &&  mouse.x>shape.ex-20 && mouse.y>shape.ey-20){
		...
		tcanvas.select='';
		tcanvas.mousedown=tcanvas.mousezoom=tcanvas.mousemove=false;
	})
	...
	if(this.valid){
		...
		if(this.mousemove){
			...
			if(shape.shape!="line" &&　shape.shape!="g"){
				ctx.fillRect(shape.ex,shape.ey,-20,-20);
			}
