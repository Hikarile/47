$(function(){
	var pen=new Pen;
	var canvas=$("#canvas")[0];
	var cnvasstart=new Canvas(pen,canvas);
	...
	$("#save2").click(function(){
		var data={shapes:cnvasstart.shapes};
	...
	$("#open2").change(function(){
		...
		reader.onload=function(event){
			...
			for(i=0;i<text.shapes.length;i++){
				...
				cnvasstart.shapes.push(shape);
				if(i==text.shapes.length-1){
					cnvasstart.valid=true;
					cnvasstart.draw();
				}
			}
		}
...
function Canvas(pen,canvas){
	...
	$(canvas).mousedown(function(e){
		...
		tcanvas.select='';
		tcanvas.mousedown=tcanvas.mousemove=tcanvas.mousezoom=false;
	})
...
Canvas.prototype.draw=function(){
	if(this.valid){
		...
		img=ctx.getImageData(0,0,this.width,this.heigth);
