...
<?php
	...
	$id=$_GET['id'];
	$mm=$mysql->query("SELECT * FROM `menu` where `id` = '$id'");
	...
	if($_POST['ok']){
		...
		if($menu==''||$money==''){
			...
			$mysql->query("UPDATE `menu` SET `menu` = '$menu', `money` = '$money' WHERE `menu`.`id` = '$id'");
        ...
        <form method="post">
        	<table width="30%">
           		<tr>
                    ...
                    <td><input type="text" name="menu" value="<?=$m['menu']?>"></td>
                </tr>
                <tr>
                	<th bgcolor="#CCCCCC">價錢</th>
                    <td><input type="number" name="money" value="<?=$m['money']?>"></td>
                </tr>
                <tr>
                	<th colspan="2">
