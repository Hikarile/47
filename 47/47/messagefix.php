...
*/		
		$a=checkInput($name,$phone,$email,$text,$number);
		if($a==''){
			$time=date("Y/m/d h:i:s");
			$mysql->query("UPDATE `message` SET `name` = '$name', `email` = '$email', `phone` = '$phone', `ey` = '$ey', `py` = '$py', `text` = '$text', `number` = '$number', `ftime` = '$time' WHERE `id` = '$id'");
...
<body bgcolor="#6699FF">
	<center><h1>
    	修改留言<input type="button" value="返回" onClick="location.href='message.php'">
        ...
        <form  method="post" enctype="multipart/form-data">
        	<table width="30%">
                ...
                <tr>
                	<?php
                    if($b['png']==''){
					echo'<th bgcolor="#999999">內容</th>
                    <td>
                    	<input type="text" name="text" value="'.$a['text'].'">
                    </td>';
					}else{
					?>
					<th colspan="2">
                ...
                <tr>
                    ...
                    <td>
                    	<input type="tel" name="number" pattern="[A-Za-z]{3}[0-9]{3}" value="<?=$a['number']?>">
                    </td>
                </tr>
                <tr>
                	<th colspan="2">
