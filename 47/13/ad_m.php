...
<?php
	...
	if($_POST['up']){
		$mysqli->query("UPDATE `message` SET `up` = '1' WHERE `message`.`id` = '".$_POST['id']."'");
		header("location:ad_m.php");
	}
	if($_POST['noup']){
		$mysqli->query("UPDATE `message` SET `up` = '0' WHERE `message`.`id` = '".$_POST['id']."'");
		header("location:ad_m.php");
	}
	if($_POST['d']){
		$mysqli->query("DELETE FROM `message` WHERE `message`.`id` = '".$_POST['id']."'");
		header("location:ad_m.php");
	}
?>
<body bgcolor="#6699FF">
	<center><h1>
		...
		<div class="d0">
            ...
            <div class="d4">
				...
				<form method="post"><p/>
					...
					?>
                    <input type="button" name="reply" value="回覆" onClick="location.href='ad_mr.php?id=<?=$a['id']?>'"><p/>
		...
		?>
		<div style="width:80%; height:60px; font-size:30px; text-align:left; background-color:#6C9;">
        	管理者回覆:<?=$c['reply']?>
        </div>
		<?php
		}
