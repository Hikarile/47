...
<?php
	...
	if($_POST['ok']){
		...
		if($a==''){
			...
			$mysqli->query("UPDATE `message` SET `name` = '$name', `email` = '$email', `phone` = '$phone', `ey` = '$ey', `py` = '$py', `text` = '$text', `number` = '$number', `ftime` = '$time' WHERE `message`.`id` = '$id'");
			...
			if($b['png']==''){
				...
				if($_FILES['png']['name']!=''){
					move_uploaded_file($_FILES['png']['tmp_name'],'file/'.$_FILES['png']['name']);
					$mysqli->query("UPDATE `png` SET `png` = '".$_FILES['png']['name']."' WHERE `png`.`mid` = '$id'");
				}
			}
	...
	<center><h1>
        ...
        <form method="post" enctype="multipart/form-data">
        	<table width="30%">
				...
				?>
				<tr>
                	<th colspan="2"><img width="30%" src="file/<?=$b['png']?>"></th>
                </tr>
				<?php
				}
				?>
