<?php
	...
	if($a==0){
		if($_POST['se']!=''){
			$sd=strtotime($ss);
			$se=strtotime($_POST['se']);
			$_SESSION['a']=($se-$sd)/3600/24/7;
		}else{
			$_SESSION['a']=0;
		}
	}else if($a==1){
	...
	$ii=$_SESSION['a']*7;
?>
<table id="big1" width="80%" border="1" bgcolor="#FFFFFF">
	<tr>	
...
<table id="big2" width="80%" border="1" bgcolor="#FFFFFF">
    ...
    <tr height="100px">
        ...
        for($i=0;$i<=6;$i++){
			...
			$cont=0;$ten=0;
			$aa=$mysqli->query("SELECT * FROM `eat` where `day` = '$day'  and `tp` = '午餐'");
			...
			if($ten<=0){
				echo'<th class="a" day="'.$day.'" tp="午餐">已無座位</th>';
			}else{
			?>
    ...
    <tr height="100px">
        ...
        for($i=0;$i<=6;$i++){
			...
			$dd=date("Ymd",strtotime(-$mon+$ii."day"));
			$cont=0;$ten=0;
			$aa=$mysqli->query("SELECT * FROM `eat` where `day` = '$day'  and `tp` = '下午茶'");
			while($a=mysqli_fetch_array($aa)){
				if($a['id']!=$id){
					$cont=$cont+$a['tab'];
				}
			...
			if($ten<=0){
				echo'<th class="a" day="'.$day.'" tp="下午茶">已無座位</th>';
			}else{
			?>
    ...
    <tr height="100px">
        ...
        for($i=0;$i<=6;$i++){
			...
			$dd=date("Ymd",strtotime(-$mon+$ii."day"));
			$cont=0;$ten=0;
			$aa=$mysqli->query("SELECT * FROM `eat` where `day` = '$day'  and `tp` = '晚餐'");
			while($a=mysqli_fetch_array($aa)){
				if($a['id']!=$id){
					$cont=$cont+$a['tab'];
				}
			...
			if($ten<=0){
				echo'<th class="a" day="'.$day.'" tp="晚餐">已無座位</th>';
			}else{
			?>
				<th class="a" day="<?=$day?>" tp="晚餐" onClick="cli('<?=$day?>','晚餐','<?=$ten?>','<?=$i?>','<?=$dd?>',this)"><?=$ten?></th>
			<?php	
			}
		}
		?>
